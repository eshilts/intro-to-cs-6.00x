...
def main():
    ...
    print "1) Prob of collision for 1000 buckets and 50 insertions: {0}".format(
            str(round(collision_prob(1000, 50), 3)))
    print "2) Prob of collision for 1000 buckets and 200 insertions: {0}".format(
            str(round(collision_prob(1000, 200), 3)))
    print "3) Prob of collision for 1000 buckets and 50 insertions with 1000 simulations: {0}".format(
            str(round(observe_prob(1000, 50, 1000), 3)))
    print "4) Prob of collision for 1000 buckets and 200 insertions with 1000 simulations: {0}".format(
            str(round(observe_prob(1000, 200, 1000), 3)))
    print "5) Prob of collision for 365 buckets and 30 insertions: {0}".format(
            str(round(collision_prob(365, 30), 3)))
    print "6) Prob of collision for 365 buckets and 250 insertions: {0}".format(
            str(round(collision_prob(365, 250), 3)))

    prob = 0.0
    size = 0
    for i in range(365):
        newProb = collision_prob(365, i)
        if newProb >= prob and newProb < 0.99:
            prob = newProb
            size = i

    print "7) Max classroom size = {0} with probability = {1}".format(
            str(size), str(round(prob, 3)))
